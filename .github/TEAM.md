...
| Lux | Visuals & art direction lead **(new)** | [agents/Lux.agent.md](agents/Lux.agent.md) | [portfolios/lux/](portfolios/lux/) |
| Jesse | Repository manager / community coordinator | [agents/Jesse.agent.md](agents/Jesse.agent.md) | _works in `.github/`_ |
...
|---|---|---|---|
| Bridge | _n/a_ | _n/a_ | Inline summaries to user |
| Sol | repo `src/`, `scripts/`, workflows | _n/a_ | Merged PRs into `development` |
| Vex | `src/data/`, `docs/wiki/`, `docs/about/` | _n/a_ | Merged PRs |
| Rook | `tests/`, `playwright-report/`, `test-results/` | screenshot evidence in PR comments | QA reports + pass/fail signals |
...
| Jesse | `.github/` (labels, milestones, board, wiki spec) | _n/a_ | Issues, board fields, release notes |
...
| ↓ from \ to → | Sol | Vex | Rook | Robert | Lux | Jesse |
|---|---|---|---|---|---|---|
| **Sol** | — | schema/interface change notes | code ready for QA | research questions | implemented surfaces for visual review | tracking issue updates |
| **Vex** | content schema needs | — | content QA requests | tone/lore research asks | narrative tone & lore for visual translation | content tracking |
| **Rook** | repro steps + diagnostics | content defects | — | _rare_ | UI defects with screenshots | QA-status / regression labels |
| **Robert** | research findings, citations | tone/setting references | _rare_ | — | reference images, mood boards (`portfolios/robert/images/`) | gap reports |
| **Lux** | finalized visual specs, UI/UX briefs, asset briefs | visual language proposals (for narrative alignment) | a11y/perf budget targets, visual deliverables for validation | reference-image asks | — | work items, label requests |
| **Jesse** | issue assignments, board fields | issue assignments | release-gate checklists | research requests | art-direction work items, label/milestone updates | — |
...
```
Robert (research) → Vex (narrative) → Lux (visuals) → Sol (implementation) → Rook (QA) → Jesse (tracking & release notes)
```
...
| `docs/design/` | Promoted design specs (from Lux/Vex) | visual style guides, UX flows |

File naming inside portfolios: `[topic]-[descriptor].[ext]` for assets; `[YYYY-MM]-[slug].md` for dated reports.

---

_Last updated: 2026-05-02. Maintained by Jesse._
