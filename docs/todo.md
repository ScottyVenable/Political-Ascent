...
27. Expand the Build Your Candidate UI and UX, specifically the Core Stats UI so that the points/scores are better represented visually and the player can understand how their choices affect their character and gameplay. — **Shipped (`exp--byc-stats-detail`).** Earlier passes added per-stat tier chip + per-tier impact blurb and the budget bar + presets (todo#28). This pass adds the **Build Profile** card under the six sliders: three derived archetypes (Persuasion = charisma + connections, Operations = strategy + integrity, Resources = wealth + stamina) each showing live score, tier chip, contributing pair, and tier-specific gameplay blurb. New `src/utils/buildProfile.ts` with 6 vitest cases; Playwright screenshot validates the card. The "save and compare different character builds" sub-suggestion is deferred — it changes save-file shape and is not in scope for this UX pass.
...
74. In the Character screen, track the player character's personal funds and implement ways to get more. — **Shipped (`exp--character-personal-funds`).** The Character panel's Personal Finances card now exposes deterministic finance actions: Salary + reimbursements, Paid speaking circuit, Liquidate assets, and Self-fund campaign. The first three add to `personalFunds`; Self-fund campaign spends personal money and transfers the same amount into campaign Treasury without overdrafting. New `src/utils/personalFinance.ts` centralizes the action math with 7 Vitest cases; character-store tests now cover `adjustFunds`; Playwright validates paid speaking + self-funding and captures `tests/e2e/__screenshots__/character-finances/personal-finances.png`.
