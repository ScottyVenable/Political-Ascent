...
- **Legislation overhaul — Phase 1** (`exp--legislation-overhaul-p1`, addresses Lead Director feedback on `copilot/create-game-roadmap-and-test-tasks`). Drafting screen evolves from a single-pass module-toggle UI into the first slice of a structured wizard. Four player-visible additions:
  - **Bill type selector** in the header — Resolution / Act / Constitutional Amendment / Appropriations. The choice flows through to `LegislationSystem.draftBill` via the new `applyBillType()` helper which adjusts opposition (`-15` / `0` / `+30` / `+5`) and per-stage PC cost (`×0.5` / `×1` / `×3` / `×1`).
  - **Aide Advisory card** — pure rules engine in `src/utils/aideAdvisory.ts` reads the synthesised draft and surfaces one contextual nudge (e.g. "Opposition above 75 — consider attaching a Sunset Clause", "Appropriations bill needs a fiscal module"). Returns `null` when nothing needs saying so the panel hides instead of showing a fake "you're doing great".
  - **Drag-to-reorder active modules** — replaces the old `Set<string>` with an ordered list. The order matters: later modules win on conflicting effect targets, and the bill-text preview honours the order.
  - **Mechanical Breakdown step** — explicit if-passes / if-fails confirmation before the bill is actually introduced. Two columns list every effect in plain English and the consequences of a defeat (PC sunk, opposition hardens, retry cooldown). The player must click *Confirm & introduce* to call `LegislationSystem.draftBill`.
  - 15 new Vitest cases (`billType` 6, `aideAdvisory` 9). Existing draft-legislation Playwright spec updated to walk the new wizard flow; new spec added that asserts the bill-type selector and the advisory tone change. New screenshot harness `legislation-overhaul-screenshots.spec.ts` captures Compose / Amendment-warning / Breakdown at three desktop viewports.
- **`docs/LEGISLATION_OVERHAUL_PLAN.md`** — four-phase plan covering Identity / Drafting / Coalition / Breakdown wizard, bill types, aide advisories, drag-to-reorder modules, talk-to-co-sponsor pitch UX, bicameral other-chamber routing, chamber leadership Whip Nudges. Phase 1 lands in this PR; phases 2–4 sequenced for follow-up.
- **`docs/DEVELOPMENT_ROADMAP.md` § Cluster F — Strategic ideas backlog.** Captures Lead Director's "spilled ideas" from the PR comment thread: bicameral routing, whip/leader nudges, turn-based reassessment (decision recorded: keep pause/play/speed; offer opt-in *Deliberate* mode that auto-pauses on events), player role selection + run-for-president path, national + world map with role-scoped diplomacy, expanded character creation with portrait upload + age + backstory, Stellaris-style game-settings step.

### Added

- **`docs/DEVELOPMENT_ROADMAP.md`** — operational roadmap that pairs `docs/ROADMAP.md` (versioned milestones) and `docs/todo.md` (raw idea backlog) with a status snapshot of the codebase, a definition of "play-test ready", a sequenced plan toward `v0.1.0-beta.1`, and explicit clusters for upcoming PRs (Communications/Dialogue, Committee/Vote roll-call, US map, tooling, content). Updated as the agent's running planning artefact between sessions.
- **PR-screenshot harness** — `tests/e2e/pr-screenshots.spec.ts` walks MainMenu → CharacterCreation → Dashboard → all twelve panels and writes captures to `tests/e2e/__screenshots__/play-test-readiness/` at four viewports (1280×720, 1440×900, 1920×1080, Pixel 7 portrait). Uses `page.screenshot` instead of `toHaveScreenshot` so first runs do not fail with "snapshot doesn't exist". Surfaces 44 screenshots per pass for review.

### Fixed

- **Typecheck floor restored** (5 errors). `src/renderer/panels/NewsPanel.tsx` was passing the full `GameDate` object to a formatter that expected `{week, year}` — switched to the canonical `formatDateShort`. `src/renderer/screens/Game.tsx` ` VoteResultModal` lost narrowing of `modal` inside the `dismiss()` closure — captured `modalId` after the early return. `src/renderer/screens/Game.tsx` `CardEffectsModal` cast loose `{type: string; ...}[]` directly to `Effect[]` — routed through `unknown` per TypeScript's overlap rule. `src/systems/CardSystem.test.ts` read `res.reason` on `PlayResult` without narrowing on `ok === false`.
- **Lint floor restored** (15 errors). `react/no-unescaped-entities` errors in `GlossaryPanel.tsx` Knowledge-Base content fixed by escaping `'` → `&apos;` and `"` → `&quot;` in marketing-copy paragraphs.
- **Playwright invocability**. `@playwright/test` was present in `package-lock.json` but missing from `package.json` so the toolchain failed with `Cannot find package '@playwright/test'`. Added it to `devDependencies` plus the standard `test:e2e` / `test:e2e:ui` / `test:e2e:update` scripts.
- **Stale e2e selectors** uncovered by the first full run on this branch:
  - `tests/e2e/glossary-panel.spec.ts` was searching for a sidebar button called `Glossary`, but todo#45 renamed the tab to **Knowledge Base** months ago. Updated the locator and the heading assertion.
  - `tests/e2e/accessibility.spec.ts` imported `../fixtures/...` which resolves to `tests/fixtures/`; the actual fixtures live at `tests/e2e/fixtures/`. Switched to `./fixtures/`.
  - `tests/e2e/mobile-portrait.spec.ts` had no project guard, so its phone-only selectors (`md:hidden` hamburger, drawer sidebar) were attempted on every desktop project and timed out. Added a `beforeEach` skip that limits the suite to `chromium-pixel-portrait`.

### Status snapshot

After this PR the workspace is at a clean play-test floor: lint and typecheck are green, all 41 vitest files (293 tests) pass, the production build produces 474 kB / 143 kB gz, and the Playwright suite at 1280×720 reports **79 pass / 3 known-flaky / 4 mobile-skipped (non-portrait projects)**. The 3 known flakes are pre-existing animation-timing issues in `cards-and-tooltip.spec.ts`, `cards-ux-hardening.spec.ts`, and `right-click-menus.spec.ts` and are explicitly tracked under §3.3 of the new `DEVELOPMENT_ROADMAP.md`.

### Added

- **Congress member ideology labels** (`exp--congress-ideology-labels`, todo#91). Congress member details now use the shared `ideologyLabel()` helper instead of exposing raw compass coordinates like `0.42 · -0.31`. Seat hover tooltips and member modals show phrases such as `Right-Libertarian`, `Moderate Left-Authoritarian`, or `Centrist`, matching the language used by the ideology compass. Added Playwright coverage that opens a Congress member modal, asserts no raw coordinate pair is present, and captures the modal. Also added the missing approved `filter` icon registry entry used by the advanced-filter accordion and removed an unused member-list prop from `CongressPanel.tsx`.

- **Character personal-finance actions** (`exp--character-personal-funds`, todo#74). The Character panel's Personal Finances card now lets players change their tracked `personalFunds` instead of only reading the number. Three deterministic income actions are available: Salary + reimbursements (wealth/background scaled), Paid speaking circuit (charisma + connections scaled), and Liquidate assets (wealth-heavy emergency cash). A fourth action, Self-fund campaign, spends personal funds and transfers the same amount into campaign Treasury without allowing an overdraft. New pure helper `src/utils/personalFinance.ts` keeps the action math testable; 7 helper tests plus 3 character-store tests cover income, spending, zero-floor behavior, and Treasury-transfer metadata. New Playwright coverage clicks paid speaking and self-funding, asserts both personal funds and top-bar Treasury, and captures the Personal Finances card.

- **Build Profile card on Core Stats step** (`exp--byc-stats-detail`, todo#27). Under the six raw stat sliders the wizard now renders a three-card "Build Profile" panel that distills the distribution into named archetypes — **Persuasion** (charisma + connections), **Operations** (strategy + integrity), **Resources** (wealth + stamina). Each card shows the live averaged score, a tier chip (Weak / Average / Strong / Exceptional) using the existing palette, the contributing stat pair, and a tier-specific gameplay-impact blurb (e.g. "You set the narrative. Coalitions form around your speeches."). The card updates every slider tick so a player sees immediately what their distribution actually shapes their character into, instead of mentally translating six numbers. New pure utility `src/utils/buildProfile.ts` with 6 Vitest cases; new Playwright spec captures the panel after skewing two sliders.

- **Canonical chart primitives + EconomyPanel polish** (`exp--graph-renderer-polish`, todo#66). New `src/renderer/components/charts/` module ships two reusable visualisations: `Sparkline` (line chart with optional area-fill gradient, latest-value dot, configurable tone palette including literal hex, optional `responsive` mode that hands sizing to CSS for stretchable variants) and `MiniBarChart` (vertical bars with rounded tops, axis labels, optional value labels, per-bar tone overrides, scaled to peak). Two near-identical local Sparkline implementations in `EconomyPanel.tsx` and `DashboardPanel.tsx` were collapsed into the canonical component — Economy trends now show area fill + last-value dot for the first time. 14 new Vitest cases (geometry, empty-state, all-equal-values divide-guard, hex pass-through, responsive sizing). New Playwright spec captures the Economy panel after 2 months of sim time so the trends card renders real data.

- **Congress demographic filters & sort** (`exp--congress-bio-filters`, todo#55). `Legislator` now carries `age`, `gender` (F/M), and `wealth` (USD); `CongressSystem` seeds them deterministically — senators skew older than house members (mean ~65 vs ~58), wealth follows a log-normal-ish curve from a ~$30k floor through ~$50M outliers. The Congress advanced-filter accordion gained Gender / Age (<50, 50–65, 65+) / Wealth (<$1M, $1–10M, $10M+) rows; the sort row gained Age, Wealth, and Bills (sponsorship count derived live from pending+passed+failed legislation). The seat tooltip and the member modal surface the new fields. Filters cooperate with the existing dim/hide hemicycle toggle. Hemicycle text fixture updated for the new shape; 2 new Vitest cases cover seeding invariants; new Playwright spec captures a desktop screenshot with the women-65+ filter applied. Todo #69 archived in the same PR as already-shipped (single-chamber view, cursor-anchored tooltip, dim/hide toggle, real-time filter all already in place).
